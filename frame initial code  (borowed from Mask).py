...
def frame_images(color, wide, original_image):
    
    width, height = original_image.size
    rounded_mask = PIL.Image.new('RGBA', (width, height), (127,0,127,0))
    drawing_layer = PIL.ImageDraw.Draw(rounded_mask)
    
    drawing_layer.polygon([(0, height),(width/8, height),(0,0),(width/8, 0)],fill=(0,0,0,1))
    
def frame_all_images(directory=None):
    """Saves a modfied version of each image in directory framed"""
    ...
    for n in range(len(image_list)):
        ...
        new_image = frame_images(image_list[n], .30)
